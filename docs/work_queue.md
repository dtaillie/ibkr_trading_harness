...
- Add an explicit "What is running right now?" strip that shows:
  ...
  - partial; runtime session evidence is now exportable through
...
- Add a more intuitive first-run experience:
  ...
  - surface the exact local config/data-root/action that would resolve each
    ...
    - partial; Data Diagnostics now includes a bounded Universe Visibility
      ...
      can launch the deeper single-symbol diagnostic. Visible rows now include
      the best catalog file plus Inspect and Workbench actions, so a pasted
      universe can move directly from visibility review into chart inspection
      or replay setup.
...
- Add a clean "current strategy performance" mode that does not require picking
  ...
  - partial; Performance Home now promotes a Current Scoreboard directly under
...
- Add a Help page and contextual help:
  ...
  - empty states that explain what to do next instead of showing blank tables
    ...
    - partial; Help Home now includes a state-aware Guided Tour that walks a
      new user through current health, performance, saved data, simulation,
      run evidence, and operations/public-boundary review in order, marking
      each step ready, warning, or blocked.
...
- Add a guided "paper monitor" path:
  ...
  - partial; Operations Home now includes an Action Summary that prioritizes
    paper readiness, Gateway/API diagnostics, remote-node freshness, command
    audit/control queue review, and local alerts with direct Paper, Remote,
    Control, and Diagnostics routes.
